...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if (message.author.id != "1199388778813464618")
    ...
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<:Builder:1430843501112856697> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<:Builder:1430843501112856697> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)
